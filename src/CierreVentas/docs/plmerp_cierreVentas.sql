...
CREATE TABLE `ig_locales` (
  ...
  `codigoArea` varchar(2) DEFAULT NULL,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='Lo normal es que todos los egresos sean aplicables en todos los locales. En esta tabla se registran las cuentas de egresos que no se deben aplicar a un local. Esta lista siempre será más corta, o quizás nula, que la de los cuentas que se usan en cada local.';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `pe_historia` (
  ...
  `idPersona` int unsigned NOT NULL,
  `fecha` date NOT NULL,
  `idLocal` int unsigned DEFAULT NULL,
  `descripcion` varchar(255) NOT NULL COMMENT 'Para registrar un ingreso o un cambio de local se pone el local, para registrar una salida, se deja el local en blanco(nulo)',
